...
export interface InventoryBatch {
  ...
  createdAt: string;
  unitCost: number; // Costo per unità (ml, g, pz) del lotto
  concentration?: number;
  // Maceration fields
  status: 'available' | 'macerating';
  macerationEndDate?: string;
  actualMacerationDays?: number;
  notes?: string;
}
...
export interface ProductionComponent {
  ...
  totalCost: number; // Costo totale del componente utilizzato
  weightInGrams?: number;
  sourceBatches: {
    batchId: string;
    quantityTaken: number;
    costContribution: number; // Costo apportato da questo specifico lotto
  }[];
}
...
export interface Production {
  ...
  totalCost: number; // Costo totale della produzione
  costPerUnit: number; // Costo per unità del prodotto finito
  
  // Usato per 'fill' e 'transform' per tracciare il lotto di origine
