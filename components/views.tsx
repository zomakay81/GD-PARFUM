...
const EditStockLoadModal: React.FC<{ isOpen: boolean, onClose: () => void, load: StockLoad | null }> = ({ isOpen, onClose, load }) => {
    ...
    const yearData = state[settings.currentYear];
    const [formData, setFormData] = useState<StockLoad | null>(null);

    useEffect(() => {
        if (load) {
            // Deep copy to avoid mutating original state directly
            setFormData(JSON.parse(JSON.stringify(load)));
        }
    }, [load, isOpen]); // Rerun on isOpen to reset changes on reopen

    if (!load || !formData) return null;
    ...
    const handleHeaderChange = (field: keyof StockLoad, value: any) => {
    ...
    const handleSave = () => {
        ...
        dispatch({
            ...
            payload: formData // Send the whole updated object
        });
        onClose();
    };

    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`Modifica Carico #${load.id.substring(0,8).toUpperCase()}`}>
             <div className="space-y-6">
                {/* Header Section */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Input type="date" label="Data" value={formData.date} onChange={e => handleHeaderChange('date', e.target.value)} />
                    <div>
                        <label className="block text-sm font-medium mb-1">Fornitore</label>
                        <select className="w-full p-2 border rounded dark:bg-gray-700 dark:border-gray-600" value={formData.supplierId} onChange={e => handleHeaderChange('supplierId', e.target.value)}>
                    ...
                    <div>
                        ...
                        <select className="w-full p-2 border rounded dark:bg-gray-700 dark:border-gray-600" value={formData.paidByPartnerId} onChange={e => handleHeaderChange('paidByPartnerId', e.target.value)}>
                ...
                <div className="border-t dark:border-gray-700 pt-4">
                    <h4 className="font-semibold mb-3 text-lg">Articoli del Carico</h4>
                    <div className="space-y-3 max-h-60 overflow-y-auto pr-2">
                        {formData.items.map((item, index) => (
                            <div key={`${item.variantId}-${index}`} className="p-3 grid grid-cols-1 md:grid-cols-5 gap-3 items-end bg-gray-50 dark:bg-gray-700/50 rounded-lg border dark:border-gray-600">
                                <div className="md:col-span-3">
                                    <label className="text-xs font-semibold text-gray-500 uppercase">Prodotto</label>
                                    <p className="text-sm font-medium truncate">{getProductName(item.variantId)}</p>
                                </div>
                                <div>
                                    <Input type="number" label="Quantità" value={item.quantity} onChange={e => handleItemChange(index, 'quantity', parseFloat(e.target.value) || 0)} step="0.01"/>
                                </div>
                                <div>
                                    <Input type="number" label="Prezzo Unit." value={item.price} onChange={e => handleItemChange(index, 'price', parseFloat(e.target.value) || 0)} step="0.0001"/>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="flex justify-end pt-4 border-t dark:border-gray-700">
                    <Button variant="secondary" onClick={onClose}>Annulla</Button>
                    <Button onClick={handleSave} className="ml-2">Salva Modifiche</Button>
                </div>
            </div>
        </Modal>
    );
...
const QuotesHistoryView: React.FC = () => { 
    ...
    const filteredQuotes = useMemo(() => {
        return yearData.quotes.filter(q => {
            if (!customerFilter) return true;
            const customerName = getCustomerName(q.customerId).toLowerCase();
            return customerName.includes(customerFilter.toLowerCase());
        });
    }, [yearData.quotes, customerFilter]);

    const groupedQuotes = useMemo(() => {
        const groups: { [key: string]: Quote[] } = {};
        filteredQuotes.forEach(quote => {
            const month = quote.date.substring(0, 7);
            if (!groups[month]) {
                groups[month] = [];
            }
            groups[month].push(quote);
        });
        return Object.entries(groups).sort(([a], [b]) => b.localeCompare(a));
    }, [filteredQuotes]);

    const grandTotal = useMemo(() => {
        return filteredQuotes.reduce((acc, q) => acc + q.total, 0);
    }, [filteredQuotes]);

    const getPaymentStatus = (quote: Quote) => {
        const paid = quote.payments ? quote.payments.reduce((acc, p) => acc + p.amount, 0) : 0;
        const remaining = quote.total - paid;
        if (remaining <= 0.01) return { text: 'Pagato', color: 'text-green-600', icon: Check };
        if (paid > 0) return { text: `Acconto (€${paid.toFixed(2)})`, color: 'text-orange-500', icon: Clock };
        return { text: 'Non Pagato', color: 'text-red-500', icon: X };
    };
            ...
            <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border dark:border-gray-700">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="md:col-span-2 text-right">
                         <h4 className="text-sm font-semibold text-gray-500">Totale Preventivi Filtrati</h4>
                         <p className="text-2xl font-bold text-primary-600">€{grandTotal.toFixed(2)}</p>
                         <p className="text-xs text-gray-400">{filteredQuotes.length} documenti</p>
                    </div>
                </div>
            </div>

            <div className="space-y-4">
                {groupedQuotes.map(([month, quotes]) => {
                    const monthTotal = quotes.reduce((acc, q) => acc + q.total, 0);
                    const isOpen = openMonth === month;
                    return (
                        <div key={month} className="border dark:border-gray-700 rounded-lg overflow-hidden">
                            <button 
                                onClick={() => setOpenMonth(isOpen ? null : month)}
                                className="w-full text-left p-4 bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 flex justify-between items-center"
                            >
                                <h3 className="font-bold text-lg">{new Date(month + '-02').toLocaleString('default', { month: 'long', year: 'numeric' })}</h3>
                                <div className="text-right">
                                    <span className="font-semibold">Totale Mese: €{monthTotal.toFixed(2)}</span>
                                    <span className="text-xs text-gray-500 block">{quotes.length} documenti</span>
                                </div>
                            </button>
                            {isOpen && (
                                <div className="p-2 animate-fade-in">
                                    <Table headers={["Numero / Data", "Cliente", "Totale", "Incassato", "Stato", "Azioni"]}> 
                                        {quotes.map(q => {
                                            const paymentStatus = getPaymentStatus(q);
                                            const StatusIcon = paymentStatus.icon;
                                            return (
                                                <tr key={q.id}> 
                                                    <td className="px-6 py-4">
                                                        <span className="block font-mono font-bold text-xs text-gray-500">#{q.id.substring(0,8).toUpperCase()}</span>
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <span className={`flex items-center font-semibold ${paymentStatus.color}`}>
                                                            <StatusIcon size={16} className="mr-1" /> {paymentStatus.text}
                                                        </span>
                                                    </td>
...
const SalesHistoryView: React.FC = () => {
    ...
    const filteredSales = useMemo(() => {
        return yearData.sales.filter(s => {
            if (!customerFilter) return true;
            const customerName = getCustomerName(s.customerId).toLowerCase();
            return customerName.includes(customerFilter.toLowerCase());
        });
    }, [yearData.sales, customerFilter]);
        ...
        <Card title="Storico Vendite">
            ...
            <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border dark:border-gray-700">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="md:col-span-2 text-right">
                         <h4 className="text-sm font-semibold text-gray-500">Totale Documenti Filtrati</h4>
                         <p className="text-2xl font-bold text-primary-600">€{grandTotal.toFixed(2)}</p>
                         <p className="text-xs text-gray-400">{filteredSales.length} documenti</p>
                    </div>
                </div>
            </div>

            <div className="space-y-4">
                {groupedSales.map(([month, sales]) => {
                    const monthTotal = sales.reduce((acc, s) => acc + s.total, 0);
                    const monthSubtotal = sales.reduce((acc, s) => acc + s.subtotal, 0);
                    const isOpen = openMonth === month;
                    return (
                        <div key={month} className="border dark:border-gray-700 rounded-lg overflow-hidden">
                            <button 
                            ...
                            >
                                ...
                                <div className="text-right">
                                    <span className="font-semibold">Totale Mese: €{monthTotal.toFixed(2)}</span>
                                    <span className="text-xs text-gray-500 block">Imponibile: €{monthSubtotal.toFixed(2)} ({sales.length} doc)</span>
                                </div>
                            </button>
                            {isOpen && (
                                <div className="p-2 animate-fade-in">
                                    <Table headers={[
                                        <div className="w-8 text-center"><CheckSquare size={16} /></div>,
                                        "Numero / Data", "Cliente", "Totale", "Stato Pagamento", "Azioni"
                                    ]}>
                                        {sales.map(s => {
                                            ...
                                            return (
                                                <tr key={s.id} className={selectedSales.has(s.id) ? "bg-blue-50 dark:bg-blue-900/20" : ""}>
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <span className="block font-mono font-bold text-xs text-gray-500">#{s.id.substring(0,8).toUpperCase()}</span>
...
const ProductLifecycleModal: React.FC<{ isOpen: boolean, onClose: () => void, variant: any | null }> = ({ isOpen, onClose, variant }) => {
    ...
    const historyData = useMemo(() => {
        ...
        for (const batch of producedBatches) {
            ...
            if (prod) {
                ...
                costDetails.push({
                    ...
                    details: {
                        maceration: batch.actualMacerationDays || prod.macerationDays,
        ...
        const adjustments = yearData.stockAdjustments.filter(a => a.variantId === variant.id);
        ...
        const timeline = [
            ...
            ...sales.map(s => ({ date: s.date, type: 'OUT_SALE', quantity: (s.items.find(i => i.variantId === variant.id)?.quantity || 0) * -1, batchNumber: '-', refId: s.id })),
            ...adjustments.map(a => ({ date: a.date, type: 'ADJUSTMENT', quantity: a.quantityAfter - a.quantityBefore, batchNumber: '-', refId: a.id, reason: a.reason }))
                                    ...
                                    {historyData.timeline.map((event, idx) => (
                                        <tr key={idx} className="border-b dark:border-gray-700">
                                            ...
                                            <td className="p-2">
                                                ...
                                                {event.type === 'OUT_SALE' && <span className="text-red-600 font-bold">Vendita</span>}
                                                {event.type === 'ADJUSTMENT' && <span className="text-orange-600 font-bold">Rettifica</span>}
                                            </td>
                                            <td className={`p-2 text-right font-mono font-bold ${event.quantity < 0 ? 'text-red-500' : 'text-green-500'}`}>
                                                {event.quantity > 0 ? '+' : ''}{event.quantity.toFixed(2)}
                                            </td>
                                            <td className="p-2 text-gray-400 font-mono">
                                                {event.type === 'OUT_SALE' 
                                                    ? `Doc #${event.refId.substring(0,6)}` 
                            ...
                            {historyData.costDetails.map((detail, idx) => {
                                ...
                                return (
                                    <div key={idx} className="p-3 border rounded-lg bg-gray-50 dark:bg-gray-800 text-sm">
                                        ...
                                        {detail.type === 'Produzione' && detail.details.concentration && (
                                            <div className="text-xs text-gray-500 mb-1">
                                                Concentrazione: <strong>{detail.details.concentration}%</strong>
                                            </div>
                                        )}
...
const StockAdjustmentModal: React.FC<{ isOpen: boolean, onClose: () => void, variant: any | null }> = ({ isOpen, onClose, variant }) => {
    const { dispatch } = useAppContext();
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`Rettifica Giacenza: ${variant.productName} - ${variant.name}`}>
            <div className="space-y-4">
                <Alert type="warning" message="Attenzione: questa operazione modifica la giacenza totale del prodotto. Usare con cautela." />
                <div className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg text-center">
                    <span className="text-sm text-gray-500">Giacenza Attuale</span>
                    <p className="text-2xl font-bold">{variant.quantity}</p>
                </div>
                <Input
                    type="number"
                    label="Nuova Giacenza Totale"
                    value={newQuantity}
                    onChange={(e) => setNewQuantity(parseFloat(e.target.value))}
                    step="0.01"
                />
                <div>
                    <label className="block text-sm font-medium mb-1">Motivazione (Obbligatoria)</label>
                    <textarea
                        value={reason}
                        onChange={(e) => setReason(e.target.value)}
                        rows={3}
                        className="w-full p-2 border rounded dark:bg-gray-800 dark:border-gray-600"
                        placeholder="Es. Conteggio inventariale, Errore di inserimento, Merce danneggiata..."
                    />
                </div>
                <div className="flex justify-end space-x-2 pt-4">
                    <Button variant="secondary" onClick={onClose}>Annulla</Button>
                    <Button onClick={handleSave}>Conferma Rettifica</Button>
                </div>
            </div>
        </Modal>
    );
...
const InventoryView: React.FC = () => {
    const { state, settings, dispatch } = useAppContext();
    const { products, productVariants, categories, inventoryBatches } = state[settings.currentYear];

    // Define a more specific type for the variants displayed in the inventory
    type DisplayVariant = ProductVariant & {
        ...
        maceratingQuantity: number;
    };

    const [isProductBaseModalOpen, setProductBaseModalOpen] = useState(false);
    const [isCategoryModalOpen, setCategoryModalOpen] = useState(false);
    const [editingVariant, setEditingVariant] = useState<ProductVariant | null>(null);
    const [deletingVariantId, setDeletingVariantId] = useState<string | null>(null);
    const [viewingImageUrl, setViewingImageUrl] = useState<string | null>(null);
    const [batchDetailVariant, setBatchDetailVariant] = useState<DisplayVariant | null>(null);
    const [lifecycleVariant, setLifecycleVariant] = useState<DisplayVariant | null>(null);
    const [isPrintModalOpen, setPrintModalOpen] = useState(false);
    const [searchTerm, setSearchTerm] = useState('');
    const [selectedCategory, setSelectedCategory] = useState('all');
    const [showProducedOnly, setShowProducedOnly] = useState(false);
    const [sortConfig, setSortConfig] = useState<{ key: 'quantity'; direction: 'ascending' | 'descending' }>({ key: 'quantity', direction: 'descending' });
    const [adjustmentVariant, setAdjustmentVariant] = useState<DisplayVariant | null>(null);

    const variantStats = useMemo(() => {
        const stats = new Map<string, { total: number, available: number, macerating: number }>();
        inventoryBatches.forEach(batch => {
            const current = stats.get(batch.variantId) || { total: 0, available: 0, macerating: 0 };
            current.total += batch.currentQuantity;
            if (batch.status === 'available') current.available += batch.currentQuantity;
            if (batch.status === 'macerating') current.macerating += batch.currentQuantity;
            stats.set(batch.variantId, current);
        });
        return stats;
    }, [inventoryBatches]);

    const displayVariants: DisplayVariant[] = useMemo(() => {
        let variants = productVariants.map(variant => {
            const product = products.find(p => p.id === variant.productId);
            const stats = variantStats.get(variant.id) || { total: 0, available: 0, macerating: 0 };
            return {
                ...variant,
                ...
                availableQuantity: stats.available,
                maceratingQuantity: stats.macerating
            };
        });

        if (selectedCategory !== 'all') {
            variants = variants.filter(v => v.category === selectedCategory);
        }
        if (showProducedOnly) {
            variants = variants.filter(v => inventoryBatches.some(b => b.variantId === v.id && b.productionId));
        }
        if (searchTerm) {
            const lowercasedFilter = searchTerm.toLowerCase();
            variants = variants.filter(v =>
                v.productName.toLowerCase().includes(lowercasedFilter) ||
                v.name.toLowerCase().includes(lowercasedFilter) ||
                v.brand.toLowerCase().includes(lowercasedFilter)
            );
        }
        if (sortConfig) {
            variants.sort((a, b) => {
        ...
        return variants;
    }, [productVariants, products, selectedCategory, searchTerm, sortConfig, variantStats, showProducedOnly, inventoryBatches]);
...
const ProductionView: React.FC = () => {
    const { state, settings, dispatch } = useAppContext();
    const [view, setView] = useState<'menu' | 'bulk' | 'direct' | 'fill' | 'transform'>('menu');

    const ActionButton = ({ icon, title, description, onClick }: { icon: React.ReactNode, title: string, description: string, onClick: () => void }) => (
        <button
            onClick={onClick}
            className="text-left p-6 bg-white dark:bg-gray-800 rounded-xl shadow-lg hover:shadow-xl hover:scale-105 border border-gray-200 dark:border-gray-700 transition-all duration-300 group"
        >
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h3 className="text-lg font-bold text-gray-900 dark:text-white group-hover:text-primary-600">{title}</h3>
    ...
    const renderMenu = () => (
        <div className="max-w-4xl mx-auto">
             <div className="text-center mb-12">
                <h2 className="text-3xl font-bold tracking-tight">Cosa vuoi produrre oggi?</h2>
                <p className="mt-4 text-lg text-gray-600 dark:text-gray-400">Seleziona un tipo di processo per iniziare.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <ActionButton 
                    icon={<Beaker size={24} />} 
                    title="Produzione Bulk" 
                    description="Crea un semilavorato da materie prime per la macerazione." 
                    onClick={() => setView('bulk')}
                />
                <ActionButton 
                    icon={<PackageIcon size={24} />} 
                    title="Produzione Diretta" 
                    description="Crea un prodotto finito e imbottigliato, pronto per la vendita." 
                    onClick={() => setView('direct')}
                />
                <ActionButton 
                    icon={<Droplets size={24} />} 
                    title="Riempimento da Bulk" 
                    description="Imbottiglia un semilavorato già macerato in un prodotto finito." 
                    onClick={() => setView('fill')}
                />
                <ActionButton 
                    icon={<ArrowRightLeft size={24} />} 
                    title="Trasformazione" 
                    description="Converti un prodotto finito in un altro formato (es. 1x100ml -> 2x50ml)." 
    ...
    const BulkProductionForm: React.FC<{ onBack: () => void }> = ({ onBack }) => {
        const { state, settings, dispatch } = useAppContext();
        const { products, productVariants, categories } = state[settings.currentYear];
        ...
        const [formData, setFormData] = useState({
            ...
            macerationDays: 30,
            components: [{ variantId: '', quantityUsed: 0 }],
        });
        const [concentration, setConcentration] = useState(20);
        const [suggestions, setSuggestions] = useState({ essence: 0, alcohol: 0 });

        const availableComponents = useMemo(() => productVariants.filter(v => {
            const product = products.find(p => p.id === v.productId);
            const category = categories.find(c => c.name === product?.category);
            return category?.isComponent;
        }), [productVariants, products, categories]);

        const bulkTargets = useMemo(() => productVariants.filter(v => {
            const product = products.find(p => p.id === v.productId);
            const category = categories.find(c => c.name === product?.category);
            return category?.name === "Semilavorato (Sfuso)";
        }), [productVariants, products, categories]);

        const targetProduct = useMemo(() => {
            if (!formData.targetVariantId) return null;
            const variant = productVariants.find(v => v.id === formData.targetVariantId);
            if (!variant) return null;
            return products.find(p => p.id === variant.productId);
        }, [formData.targetVariantId, productVariants, products]);

        useEffect(() => {
            const totalQuantity = formData.quantityProduced || 0;
            const conc = concentration || 0;
            if (totalQuantity > 0 && conc > 0) {
                const essenceAmount = totalQuantity * (conc / 100);
                const alcoholAmount = totalQuantity - essenceAmount;
                setSuggestions({ essence: essenceAmount, alcohol: alcoholAmount });
            } else {
                setSuggestions({ essence: 0, alcohol: 0 });
            }
        }, [formData.quantityProduced, concentration]);

        const getVariantDisplayName = (variant: ProductVariant) => {
            const product = products.find(p => p.id === variant.productId);
            return `${product?.name || 'Sconosciuto'} - ${variant.name}`;
        };

        const handleComponentChange = (index: number, field: 'variantId' | 'quantityUsed', value: string | number) => {
            const newComponents = [...formData.components];
            newComponents[index] = { ...newComponents[index], [field]: value };
            setFormData(prev => ({ ...prev, components: newComponents }));
        };
        
        const addComponent = () => setFormData(prev => ({ ...prev, components: [...prev.components, { variantId: '', quantityUsed: 0 }] }));
        const removeComponent = (index: number) => setFormData(prev => ({ ...prev, components: prev.components.filter((_, i) => i !== index) }));

        const handleSubmit = (e: React.FormEvent) => {
            e.preventDefault();
            if (!formData.targetVariantId || formData.quantityProduced <= 0 || formData.components.some(c => !c.variantId || c.quantityUsed <= 0)) {
                alert("Compila tutti i campi obbligatori, inclusa una quantità valida per i componenti.");
                return;
            }
            dispatch({
                type: 'ADD_PRODUCTION',
                payload: { 
                    ...formData, 
                    productionType: 'bulk',
                    concentration: concentration, // Pass concentration
                }
            });
            alert("Produzione Bulk registrata!");
            onBack();
        };

        return (
            <div className="animate-fade-in">
                <Button onClick={onBack} variant="secondary" className="mb-6"><ArrowRight className="mr-2 rotate-180" size={16}/> Torna al Menu</Button>
                <Card title="Nuova Produzione Bulk">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <Input type="date" label="Data" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} />
                             <div>
                               <label className="block text-sm font-medium mb-1">Semilavorato da Produrre</label>
                               <select className="w-full p-2 border rounded" value={formData.targetVariantId} onChange={e => setFormData({...formData, targetVariantId: e.target.value})} required>
                                   <option value="">Seleziona...</option>
                                   {bulkTargets.map(v => <option key={v.id} value={v.id}>{getVariantDisplayName(v)}</option>)}
                               </select>
                            </div>
                        </div>
                        
                        <div className="p-4 border rounded-lg bg-gray-50 dark:bg-gray-700/50 grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                            <Input type="number" label="Quantità Totale da produrre (g)" value={formData.quantityProduced} onChange={e => setFormData({...formData, quantityProduced: parseFloat(e.target.value) || 0})} required min="0.01" step="0.01"/>
                            <div>
                                <Input 
                                    type="number" 
                                    label="Concentrazione Essenza (%)" 
                                    value={concentration} 
                                    ...
                                    step="0.1"
                                />
                                {targetProduct?.ifraLimit && (
                                    <p className="text-xs text-amber-600 mt-1">
                                        <Info size={12} className="inline mr-1"/> Limite IFRA per "{targetProduct.name}": <strong>{targetProduct.ifraLimit}%</strong>
                                    </p>
                                )}
                            </div>
                        </div>

                        {suggestions.essence > 0 && (
                            <div className="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                                <h4 className="font-semibold text-blue-800 dark:text-blue-200 mb-2 flex items-center"><Beaker size={16} className="mr-2"/> Suggerimento Composizione</h4>
                                <div className="grid grid-cols-2 gap-4 text-sm">
                                    <p>Essenza: <strong className="font-mono text-lg">{suggestions.essence.toFixed(2)} g</strong></p>
                                    <p>Alcol: <strong className="font-mono text-lg">{suggestions.alcohol.toFixed(2)} g</strong></p>
                                </div>
                            </div>
                        )}

                        <Input type="number" label="Giorni di Macerazione" value={formData.macerationDays} onChange={e => setFormData({...formData, macerationDays: parseInt(e.target.value)})} required min="0" />
                        
                        <div className="border-t pt-4">
                            <h3 className="font-semibold mb-2">Componenti da Scaricare</h3>
                            <p className="text-xs text-gray-500 mb-3">Seleziona i lotti di materie prime (essenza, alcol, ecc.) che verranno usati in questa produzione. Le quantità verranno scalate dalla giacenza.</p>
                             {formData.components.map((comp, idx) => (
                                <div key={idx} className="flex gap-2 items-end mb-2">
                                    <div className="flex-grow">
                                        <label className="text-xs">Componente</label>
                                        <select className="w-full p-2 border rounded" value={comp.variantId} onChange={e => handleComponentChange(idx, 'variantId', e.target.value)} required>
                                            <option value="">Seleziona...</option>
                                            {availableComponents.map(v => <option key={v.id} value={v.id}>{getVariantDisplayName(v)}</option>)}
                                        </select>
                                    </div>
                                    <div className="w-32">
                                        <Input type="number" label="Qtà Usata (g)" value={comp.quantityUsed} onChange={e => handleComponentChange(idx, 'quantityUsed', parseFloat(e.target.value) || 0)} required min="0.01" step="0.01"/>
                                    </div>
                                    <Button type="button" variant="danger" onClick={() => removeComponent(idx)}><Trash2 size={16}/></Button>
                                </div>
                            ))}
                            <Button type="button" variant="secondary" onClick={addComponent}><Plus size={16} className="mr-2"/> Aggiungi Componente</Button>
                        </div>

                        <div className="flex justify-end pt-4">
                            <Button type="submit">Avvia Produzione Bulk</Button>
                        </div>
                    </form>
                </Card>
            </div>
    ...
    const DirectProductionForm: React.FC<{ onBack: () => void }> = ({ onBack }) => {
        const { state, settings, dispatch } = useAppContext();
        const { products, productVariants, categories } = state[settings.currentYear];
        ...
        const [formData, setFormData] = useState({
            ...
            quantityProduced: 1,
            macerationDays: 30,
            skipMaceration: false,
            components: [{ variantId: '', quantityUsed: 1 }],
            colorCode: '#000000',
            colorDrops: 0,
            enableColor: false
        });
        ...
        const directTargets = useMemo(() => productVariants.filter(v => {
            const product = products.find(p => p.id === v.productId);
            const category = categories.find(c => c.name === product?.category);
            return category?.isFinishedProduct;
        }), [productVariants, products, categories]);
        ...
        const addComponent = () => setFormData(prev => ({ ...prev, components: [...prev.components, { variantId: '', quantityUsed: 1 }] }));
        const removeComponent = (index: number) => setFormData(prev => ({ ...prev, components: prev.components.filter((_, i) => i !== index) }));

        const handleSubmit = (e: React.FormEvent) => {
            e.preventDefault();
            if (!formData.targetVariantId || formData.quantityProduced <= 0 || formData.components.some(c => !c.variantId)) {
            ...
            dispatch({
                ...
                payload: { 
                    ...
                    macerationDays: formData.skipMaceration ? 0 : formData.macerationDays,
                    colorCode: formData.enableColor ? formData.colorCode : undefined,
                    colorDrops: formData.enableColor ? formData.colorDrops : undefined,
                }
            });
            alert("Produzione Diretta registrata!");
            onBack();
        };
        ...
        return (
            <div className="animate-fade-in">
                <Button onClick={onBack} variant="secondary" className="mb-6"><ArrowRight className="mr-2 rotate-180" size={16}/> Torna al Menu</Button>
                <Card title="Nuova Produzione Diretta (Prodotto Finito)">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <Input type="date" label="Data" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} />
                            <div>
                               <label className="block text-sm font-medium mb-1">Prodotto Finito da Creare</label>
                               <select className="w-full p-2 border rounded" value={formData.targetVariantId} onChange={e => setFormData({...formData, targetVariantId: e.target.value})} required>
                                   <option value="">Seleziona...</option>
                                   {directTargets.map(v => <option key={v.id} value={v.id}>{getVariantDisplayName(v)}</option>)}
                               </select>
                            </div>
                            <Input type="number" label="Quantità da produrre (pz)" value={formData.quantityProduced} onChange={e => setFormData({...formData, quantityProduced: parseInt(e.target.value)})} required min="1" step="1"/>
                        </div>
                        
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                                <label className="flex items-center space-x-2">
                                    <input type="checkbox" checked={formData.skipMaceration} onChange={e => setFormData(p => ({...p, skipMaceration: e.target.checked}))} />
                                    <span>Salta Macerazione (Disponibile subito)</span>
                                </label>
                             ...
                             <div className="space-y-2">
                                <label className="flex items-center space-x-2">
                                    <input type="checkbox" checked={formData.enableColor} onChange={e => setFormData(p => ({...p, enableColor: e.target.checked}))} />
                                    <span>Aggiungi Colorante</span>
                                </label>
                                {formData.enableColor && (
                                <div className="flex gap-2">
                                    <Input type="color" label="Colore" value={formData.colorCode} onChange={e => setFormData({...formData, colorCode: e.target.value})} />
                                    <Input type="number" label="Gocce" value={formData.colorDrops} onChange={e => setFormData({...formData, colorDrops: parseInt(e.target.value)})} min="0" />
                                </div>
                                )}
                            </div>
                        </div>
                            ...
                            <h3 className="font-semibold mb-2">Componenti</h3>
                             {formData.components.map((comp, idx) => (
                                <div key={idx} className="flex gap-2 items-end mb-2">
                                    ...
                                    <div className="w-32">
                                        <Input type="number" label="Qtà Usata" value={comp.quantityUsed} onChange={e => handleComponentChange(idx, 'quantityUsed', parseFloat(e.target.value))} required min="0.001" step="0.001"/>
                                    </div>
                                    <Button type="button" variant="danger" onClick={() => removeComponent(idx)}><Trash2 size={16}/></Button>
                                </div>
                            ))}
                        ...
                        <div className="flex justify-end pt-4">
                            <Button type="submit">Avvia Produzione Diretta</Button>
                        </div>
                    </form>
                </Card>
            </div>
    ...
    const FillFromBulkForm: React.FC<{ onBack: () => void }> = ({ onBack }) => {
        const { state, settings, dispatch } = useAppContext();
        const { products, productVariants, inventoryBatches } = state[settings.currentYear];
        ...
        const availableBulkBatches = useMemo(() => inventoryBatches.filter(b => {
            const variant = productVariants.find(v => v.id === b.variantId);
            const product = products.find(p => p.id === variant?.productId);
            return product?.category === "Semilavorato (Sfuso)" && b.status === 'available' && b.currentQuantity > 0;
        }), [inventoryBatches, productVariants, products]);

        const fillTargets = useMemo(() => productVariants.filter(v => {
            const product = products.find(p => p.id === v.productId);
            return product?.category === "Prodotto Finito";
        ...
        const handleSubmit = (e: React.FormEvent) => {
            e.preventDefault();
            if (!formData.targetVariantId || !formData.sourceBatchId || formData.quantityProduced <= 0) {
                alert("Compila tutti i campi obbligatori.");
                return;
            }
            dispatch({
                ...
                payload: { 
                    ...formData, 
                    productionType: 'fill',
                    // Components are derived from the source batch in the reducer, so we pass an empty array
                    components: [], 
                }
            });
            alert("Riempimento registrato!");
            onBack();
        };
        
        const selectedBulkBatch = inventoryBatches.find(b => b.id === formData.sourceBatchId);
        const selectedBulkVariant = productVariants.find(v => v.id === selectedBulkBatch?.variantId);
        const maxFillable = selectedBulkVariant && formData.targetVariantId ? Math.floor(selectedBulkBatch.currentQuantity / (productVariants.find(v=>v.id === formData.targetVariantId)?.capacity || 1)) : 0;


        return (
            <div className="animate-fade-in">
                <Button onClick={onBack} variant="secondary" className="mb-6"><ArrowRight className="mr-2 rotate-180" size={16}/> Torna al Menu</Button>
                <Card title="Nuovo Riempimento da Semilavorato (Bulk)">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-medium mb-1">Lotto di Semilavorato da Usare</label>
                                <select className="w-full p-2 border rounded" value={formData.sourceBatchId} onChange={e => setFormData({...formData, sourceBatchId: e.target.value})} required>
                                    <option value="">Seleziona lotto bulk...</option>
                                    {availableBulkBatches.map(b => {
                                        const v = productVariants.find(pv => pv.id === b.variantId);
                                        return <option key={b.id} value={b.id}>{getVariantDisplayName(v!)} (Lotto: {b.batchNumber}, Disp: {b.currentQuantity})</option>
                                    })}
                                </select>
                            </div>
                            <Input type="date" label="Data Riempimento" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} />
                        </div>
                        
                        <div className="border-t pt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                               <label className="block text-sm font-medium mb-1">Prodotto Finito da Imbottigliare</label>
                               <select className="w-full p-2 border rounded" value={formData.targetVariantId} onChange={e => setFormData({...formData, targetVariantId: e.target.value})} required>
                                   <option value="">Seleziona prodotto finito...</option>
                                   {fillTargets.map(v => <option key={v.id} value={v.id}>{getVariantDisplayName(v)}</option>)}
                               </select>
                            </div>
                            <Input type="number" label="Quantità da produrre (pz)" value={formData.quantityProduced} onChange={e => setFormData({...formData, quantityProduced: parseInt(e.target.value)})} required min="1" step="1" max={maxFillable > 0 ? maxFillable : undefined} helperText={maxFillable > 0 ? `Max: ${maxFillable} pz` : ''}/>
                        </div>

                        <div className="flex justify-end pt-4">
                            <Button type="submit">Crea Prodotti Finiti</Button>
                        </div>
                    </form>
                </Card>
            </div>
    ...
    const TransformProductionForm: React.FC<{ onBack: () => void }> = ({ onBack }) => {
        const { state, settings, dispatch } = useAppContext();
        const { products, productVariants, inventoryBatches } = state[settings.currentYear];
        ...
        const availableSourceBatches = useMemo(() => inventoryBatches.filter(b => {
            const variant = productVariants.find(v => v.id === b.variantId);
            const product = products.find(p => p.id === variant?.productId);
            // Can transform any available finished product
            return product?.category === "Prodotto Finito" && b.status === 'available' && b.currentQuantity > 0;
        }), [inventoryBatches, productVariants, products]);

        const transformTargets = useMemo(() => productVariants.filter(v => {
            const product = products.find(p => p.id === v.productId);
            return product?.category === "Prodotto Finito";
        }), [productVariants, products]);
        ...
        const handleSubmit = (e: React.FormEvent) => {
            ...
            dispatch({
                ...
                payload: { 
                    ...formData, 
                    productionType: 'transform',
                    components: [], // Logic is handled by sourceBatchId
                }
            });
            alert("Trasformazione registrata!");
            onBack();
        };
        ...
        return (
            <div className="animate-fade-in">
                <Button onClick={onBack} variant="secondary" className="mb-6"><ArrowRight className="mr-2 rotate-180" size={16}/> Torna al Menu</Button>
                <Card title="Trasformazione Prodotto Finito">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-medium mb-1">Lotto di Origine da Trasformare</label>
                                <select className="w-full p-2 border rounded" value={formData.sourceBatchId} onChange={e => setFormData({...formData, sourceBatchId: e.target.value})} required>
                            ...
                            <Input type="date" label="Data Trasformazione" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} />
                        </div>
                        
                        <div className="border-t pt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                               <label className="block text-sm font-medium mb-1">Nuovo Formato (Prodotto Finito)</label>
                               <select className="w-full p-2 border rounded" value={formData.targetVariantId} onChange={e => setFormData({...formData, targetVariantId: e.target.value})} required>
                                   <option value="">Seleziona nuovo formato...</option>
                                   {transformTargets.map(v => <option key={v.id} value={v.id}>{getVariantDisplayName(v)}</option>)}
                               </select>
                            </div>
                            <Input type="number" label="Quantità Ottenuta (pz)" value={formData.quantityProduced} onChange={e => setFormData({...formData, quantityProduced: parseInt(e.target.value)})} required min="1" step="1"/>
                        </div>

                        <div className="flex justify-end pt-4">
                            <Button type="submit">Esegui Trasformazione</Button>
                        </div>
                    </form>
                </Card>
            </div>
    ...
    return (
        <div>
            ...
            {view === 'bulk' && <BulkProductionForm onBack={() => setView('menu')} />}
            {view === 'direct' && <DirectProductionForm onBack={() => setView('menu')} />}
            {view === 'fill' && <FillFromBulkForm onBack={() => setView('menu')} />}
            {view === 'transform' && <TransformProductionForm onBack={() => setView('menu')} />}
        </div>
    );
};
...
const StockLoadHistoryView: React.FC = () => {
    ...
    const filteredLoads = useMemo(() => {
        return yearData.stockLoads.filter(sl => {
            if (!supplierFilter) return true;
            const supplierName = getSupplierName(sl.supplierId).toLowerCase();
            return supplierName.includes(supplierFilter.toLowerCase());
        });
    }, [yearData.stockLoads, supplierFilter]);

    const groupedLoads = useMemo(() => {
        const groups: { [key: string]: StockLoad[] } = {};
        filteredLoads.forEach(load => {
            const month = load.date.substring(0, 7);
            if (!groups[month]) groups[month] = [];
            groups[month].push(load);
        });
        return Object.entries(groups).sort(([a], [b]) => b.localeCompare(a));
    }, [filteredLoads]);

    const grandTotal = useMemo(() => {
        return filteredLoads.reduce((acc, sl) => acc + sl.total, 0);
    }, [filteredLoads]);
        ...
        <Card title="Storico Carichi">
            <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border dark:border-gray-700">
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="md:col-span-2 text-right">
                         <h4 className="text-sm font-semibold text-gray-500">Totale Carichi Filtrati</h4>
                         <p className="text-2xl font-bold text-primary-600">€{grandTotal.toFixed(2)}</p>
                         <p className="text-xs text-gray-400">{filteredLoads.length} documenti</p>
                    </div>
                </div>
            </div>

            <div className="space-y-4">
                {groupedLoads.map(([month, loads]) => {
                    const monthTotal = loads.reduce((acc, l) => acc + l.total, 0);
                    const isOpen = openMonth === month;
                    return (
                        <div key={month} className="border dark:border-gray-700 rounded-lg overflow-hidden">
                             <button 
                            ...
                            >
                                ...
                                <div className="text-right">
                                    <span className="font-semibold">Totale Mese: €{monthTotal.toFixed(2)}</span>
                                    <span className="text-xs text-gray-500 block">{loads.length} documenti</span>
                                </div>
                            </button>
                            {isOpen && (
                                <div className="p-2 animate-fade-in">
...
const CellarView: React.FC = () => {
    ...
    const openWithdrawalModal = (batch: InventoryBatch) => {
    ...
    const handleWithdrawal = () => {
        if (selectedBatch && withdrawalAmount > 0) {
            dispatch({
                type: 'WITHDRAW_PARTIAL_MACERATION',
                payload: {
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {maceratingBatches.map(batch => {
                    ...
                    const startDate = new Date(batch.createdAt);
                    ...
                    const totalDaysMacerating = Math.ceil((today.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24));
                    ...
                    return (
                        <div key={batch.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 flex flex-col relative">
                            ...
                            <div className="space-y-3 mb-6">
                                ...
                                <div className="flex justify-between items-center text-sm">
                                    <span className="text-gray-500">Fine Prevista:</span>
                                ...
                                <div className="flex justify-between items-center text-sm pt-2 border-t dark:border-gray-700">
                                    ...
                                    <span className={`font-bold flex items-center ${isReady ? 'text-green-600' : 'text-amber-600'}`}>
                                        ...
                                        {isReady ? `Pronto (Macerato per ${totalDaysMacerating} gg)` : `${diffDays} giorni rimasti`}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="mt-auto space-y-2">
                                <Button 
                                    onClick={() => openWithdrawalModal(batch)}
                                    variant="secondary"
                                    className="w-full"
                                >
                                    Preleva Parzialmente
                                </Button>
                                <Button 
            ...
            {withdrawalModalOpen && selectedBatch && (
                <Modal 
                    isOpen={withdrawalModalOpen} 
                    onClose={() => setWithdrawalModalOpen(false)}
                    title="Prelievo Parziale"
                >
                    <div className="space-y-4">
                        <p>Quanto prodotto vuoi prelevare dal lotto <span className="font-bold">{selectedBatch.batchNumber}</span>?</p>
                        <Input 
                            type="number"
                        ...
                        <div className="flex justify-end space-x-2">
                            <Button variant="secondary" onClick={() => setWithdrawalModalOpen(false)}>Annulla</Button>
                            <Button onClick={handleWithdrawal}>Conferma</Button>
                        </div>
                    </div>
                </Modal>
            )}
